...
final class MarkChatDeliveredController
{
    #[Route('/api/chats/{chatId}/delivered', name: 'chat_mark_delivered', methods: ['POST'])]
    public function __invoke(
        ...
        $id = $data['last_delivered_message_id'] ?? null;

        if (!is_string($id) || $id === '') {
            return new JsonResponse(['error' => 'last_delivered_message_id is required'], 400);
        }

        try {
            $deliveredUuid = Uuid::fromString($id);
        } catch (\Throwable) {
        ...
        // анти-rollback по времени (как в read)
        $prev = $membership->getLastDeliveredMessageId();
        if ($prev) {
            ...
            if ($prevMsg && $prevMsg->getCreatedAt() && $msg->getCreatedAt()) {
                if ($msg->getCreatedAt() < $prevMsg->getCreatedAt()) {
                    return new JsonResponse(['error' => 'cannot move delivered backwards'], 409);
                }
            }
        }

        $membership->setLastDeliveredMessageId($deliveredUuid);
        $em->flush();
